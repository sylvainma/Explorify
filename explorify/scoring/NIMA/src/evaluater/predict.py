...
from src.handlers.data_generator import TestDataGenerator, ClusteringDataGenerator
...
def predict(model, data_generator, multiprocess=True):
    return model.predict_generator(data_generator, workers=8, use_multiprocessing=multiprocess, verbose=1)
...
#DEF Samples is a list of dictionaries, where one of the entries is an image
def process_samples(base_model_name, weights_file, samples, img_format='jpg'):
    ...
    # initialize data generator
    data_generator = ClusteringDataGenerator(samples, "", 64, 10, nima.preprocessing_function(),
                                       img_format=img_format)
    # get predictions
    predictions = predict(nima.nima_model, data_generator, multiprocess=False)
